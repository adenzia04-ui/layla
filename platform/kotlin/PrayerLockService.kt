...
class PrayerLockService : Service() {
    ...
    companion object {
        ...
        /** Backed off cadence once the 30 minutes have elapsed. */
        private const val OVERDUE_POLL_INTERVAL_MS = 8_000L
    ...
    private val poll = object : Runnable {
        override fun run() {
            checkForeground()
            // Never stops on its own. The window closing does not release the
            // user — only a completed two-step confirmation does, via ACTION_STOP.
            // Polling backs off once the window has passed so an unconfirmed
            // prayer does not sit at a 2-second cadence all day.
            val interval = if (System.currentTimeMillis() >= endsAt) {
    ...
    private fun buildNotification(): Notification {
        ...
        return builder
            .setContentTitle("$label — apps paused")
            .setContentText("Confirm your prayer in Noor to unpause them.")
